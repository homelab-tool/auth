...
async function handleWebAuthn2FA(sessionId) {
  try {
    ...
    if (!res1.ok) { showError(await res1.text()); return; }
...
async function handleTOTP(e) {
  ...
  const sessionId = /** @type {HTMLInputElement} */ (document.getElementById("totp-session-id")).value;
...
async function handleRegisterOpaque(e) {
  ...
  const clientId = /** @type {HTMLInputElement} */ (document.getElementById("reg-clientId")).value;
  const displayName = /** @type {HTMLInputElement} */ (document.getElementById("reg-displayName")).value;
...
async function handleRegisterWebAuthn(e) {
  ...
  const displayName = /** @type {HTMLInputElement} */ (document.getElementById("webauthn-displayName")).value;
